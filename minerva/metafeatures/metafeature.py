...
class MetaFeatureValue(object):
    ...
    def is_metafeature(self):
        return self.type_ == 'METAFEATURE'

    def to_arff_row(self):
        if self.is_metafeature():
            value = self.value
        else:
            value = "?"
...
class DatasetMetafeatures(object):
    def __init__(self, dataset_name, metafeature_values: typing.Dict[str, MetaFeatureValue]):
    ...
    def to_dict(self):
        return {mf.name: mf.value for mf in self.metafeature_values.values() if mf.is_metafeature()}
